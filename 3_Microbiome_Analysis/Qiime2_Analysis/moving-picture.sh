...
do
  case "${flag}" in
    ...
    ?) echo "script usage: $(basename \$0) [-m metadata file] [-s directory that contains sequence and barcode fastq files] [-b body site] [-c classifier file] [-o output directory]"
      exit 1 ;;
  esac
done
#shift "$(($OPTIND -1))"
...
qiime tools import --type EMPSingleEndSequences --input-path ${seq_m_dir} --output-path ${WDir_OutPut}/emp-single-end-sequences.qza

qiime demux emp-single --i-seqs  ${WDir_OutPut}/emp-single-end-sequences.qza --m-barcodes-file ${meta} --m-barcodes-column barcode-sequence --o-per-sample-sequences ${WDir_OutPut}/demux.qza --o-error-correction-details ${WDir_OutPut}/demux-details.qza

qiime dada2 denoise-single --i-demultiplexed-seqs ${WDir_OutPut}/demux.qza --p-trim-left 0 --p-trunc-len 120 --o-representative-sequences ${WDir_OutPut}/rep-seqs-dada2.qza --o-table ${WDir_OutPut}/table-dada2.qza --o-denoising-stats ${WDir_OutPut}/stats-dada2.qza
